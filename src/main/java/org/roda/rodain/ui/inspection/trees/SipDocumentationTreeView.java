  ...
  private void setOnDragDropped(final InspectionTreeCell cell) {
    ...
    cell.setOnDragDropped(event -> {
      ...
      if (db.hasFiles()) {
        ...
        RodaInApplication.getInspectionPane().addDocumentationToSIP(targetRaw, paths);
      } else {
        if (db.hasString()) {
          if (db.getString().startsWith("item")) {
            InspectionTreeCell targetCell = (InspectionTreeCell) event.getGestureTarget();
            TreeItem targetRaw = targetCell.getTreeItem();
            List<InspectionTreeItem> selectedItems = RodaInApplication.getInspectionPane().getDocumentationSelectedItems();
            ...
            if (db.getString().startsWith("source")) {
              InspectionTreeCell targetCell = (InspectionTreeCell) event.getGestureTarget();
              RodaInApplication.getInspectionPane().addDocumentationToSIP(targetCell.getTreeItem());
            } else
              success = false;
          }
        }
